    ...
    while index < len(lines):
        ...
        if 1 <= len(compact) <= 3 and compact.isalpha() and compact.isupper() and center_delta <= page_width * 0.18:
            ...
            if sum(len(fragment) for fragment in letters) >= 4:
                merged.append(TextLine(text="".join(letters), bbox=(x0, y0, x1, y1), font_size=font_size, bold=bold))
                index = next_index
                continue
...
def process_page(
    ...
    for obj in objects:
        ...
        if is_heading_line(line, median_font, page_width):
            flush()
            items.append(DocItem(kind="heading", page_num=page_num, text=line.text, level=heading_level(line, median_font)))
            continue
        ...
        if ordered:
            ...
            if remainder:
                flush()
                current = DocItem(kind="ol_item", page_num=page_num, text=remainder, list_style=style, ordinal=ordinal)
                previous_line = line
            else:
                flush()
                current = DocItem(kind="ol_item", page_num=page_num, text="", list_style=style, ordinal=ordinal)
                previous_line = line
            continue
        ...
        if bullet:
            flush()
            current = DocItem(kind="ul_item", page_num=page_num, text=bullet.group(1).strip())
            previous_line = line
            continue

        if current is None:
            current = DocItem(kind="p", page_num=page_num, text=line.text)
            previous_line = line
            continue
...
def render_body(items: list[DocItem]) -> str:
    ...
    for item in items:
        if item.kind == "heading":
            ...
            parts.append(f"<h{level}>{format_inline_text(item.text or '')}</h{level}>")
            continue
        ...
        if item.kind == "ul_item":
            if open_list_kind != "ul":
                ...
                open_list_kind = "ul"
            parts.append(f"<li>{format_inline_text(item.text or '')}</li>")
            continue
