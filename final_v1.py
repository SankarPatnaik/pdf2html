...
WATERMARK_LUMINANCE_THRESHOLD = 170
WATERMARK_MIN_FONT_SIZE = 22.0
WATERMARK_MAX_TEXT_LEN = 80
SUBHEADING_MAX_WORDS = 14
...
def color_luminance(color: int) -> float:
    r = (color >> 16) & 0xFF
    g = (color >> 8) & 0xFF
    b = color & 0xFF
...
def title_case_word_ratio(text: str) -> float:
    words = re.findall(r"[A-Za-z][A-Za-z'\-]*", text)
    if not words:
        return 0.0
    titled = sum(1 for word in words if word[:1].isupper() and word[1:] == word[1:].lower())
...
def table_metrics(rows: list[list[str | None]]) -> dict[str, float] | None:
    ...
    row_word_counts = [
        len(" ".join(cell for cell in row if cell).split())
        for row in cleaned_rows
        if any(cell for cell in row)
    ]
    avg_row_word_count = sum(row_word_counts) / len(row_word_counts) if row_word_counts else 0.0
    return {
        ...
        "avg_row_word_count": avg_row_word_count,
    }


def table_looks_like_flowing_text(rows: list[list[str | None]], metrics: dict[str, float]) -> bool:
    cleaned_rows = [[normalize_text(cell or "") for cell in row] for row in rows if row]
    if not cleaned_rows:
        return False
    ...
    avg_words = sum(len(row.split()) for row in row_texts) / len(row_texts)
    multi_cell_ratio = multi_cell_rows / len(row_texts)
    continuation = 0
    comparable = 0
    for previous, current in zip(row_texts, row_texts[1:]):
        previous_words = len(previous.split())
        current_words = len(current.split())
        if previous_words < 3 or current_words < 2:
            continue
        comparable += 1
        current_lead = current.lstrip("\"'“”‘’(")
        if not ends_sentence(previous) and (current_lead[:1].islower() or CONNECTOR_RE.match(current_lead)):
            continuation += 1

    continuation_ratio = (continuation / comparable) if comparable else 0.0
    return (
        column_count >= 4
        and metrics["row_count"] >= 3
        and avg_words >= 6
        and multi_cell_ratio >= 0.6
        and continuation_ratio >= 0.45
    )
...
def extract_tables(page: fitz.Page) -> tuple[list[PageObject], list[tuple[float, float, float, float]], bool]:
    ...
    for table in table_finder.tables:
        ...
        if not metrics:
            continue
        if table_looks_like_flowing_text(rows, metrics):
            continue
...
def is_watermark_like_line(
    ...
    cleaned = normalize_text(text)
    if not cleaned or len(cleaned) > WATERMARK_MAX_TEXT_LEN:
        return False
    if font_size < WATERMARK_MIN_FONT_SIZE:
        return False
    if color_luminance(color) < WATERMARK_LUMINANCE_THRESHOLD:
        return False
    centered = abs(((bbox[0] + bbox[2]) / 2) - (page_width / 2)) <= page_width * 0.18
    compact_width = (bbox[2] - bbox[0]) <= page_width * 0.7
    mostly_caps = cleaned.upper() == cleaned and any(char.isalpha() for char in cleaned)
    return centered and compact_width and mostly_caps
...
def extract_text_lines(
    ...
    for block in raw["blocks"]:
        ...
        for line in block["lines"]:
            ...
            dominant_span = max(spans, key=lambda span: float(span.get("size", 0.0) or 0.0))
            dominant_color = int(dominant_span.get("color", 0) or 0)
            if is_watermark_like_line(plain_text, bbox, font_size, dominant_color, page_width):
                continue
...
def page_has_complex_layout(lines: list[TextLine], page_num: int, page_width: float) -> bool:
    if not lines or page_num > 3:
        return False

    long_body_lines = [line for line in lines if len(line.text) >= 55 and line.x0 <= page_width * 0.2]
    if page_num <= 2 and len(long_body_lines) >= 3:
        return False
    ...
    special_layout_hits = sum(
        1
        for line in lines
        if re.search(r"(present:|coram:|hon'?ble|through:|via video conferencing)", line.text, re.I)
    )
    keyword_hits = sum(
        1
        for line in lines
        if re.search(
            r"(through|versus|appellant|respondent|petitioner|plaintiff|defendant|decree holder|judgment pronounced on|date of decision|reserved on|pronounced on)",
            line.text,
            re.I,
        )
    )
...
def is_subheading_line(
    previous_line: TextLine | None,
    line: TextLine,
    next_line: TextLine | None,
    median_font: float,
    page_width: float,
) -> bool:
    ...
    if ORDERED_MARKER_RE.match(text) or BULLET_MARKER_RE.match(text):
        return False
    if TOC_TITLE_RE.fullmatch(text) or TOC_ENTRY_RE.match(text) or TOC_DOT_LEADER_RE.match(text):
        return False
    if text.upper() == text and any(char.isalpha() for char in text):
        return False

    words = re.findall(r"[A-Za-z][A-Za-z'\-]*", text)
    if len(words) < 2 or len(words) > SUBHEADING_MAX_WORDS:
        return False

    centered = abs(((line.x0 + line.x1) / 2) - (page_width / 2)) <= page_width * 0.18
    compact_width = (line.x1 - line.x0) <= page_width * 0.7
    title_ratio = title_case_word_ratio(text)
    larger = line.font_size >= median_font * 1.03
    gap_above = max(0.0, line.y0 - previous_line.y1) if previous_line is not None else line.font_size * 3
    gap_below = max(0.0, next_line.y0 - line.y1) if next_line is not None else line.font_size * 3
    isolated = gap_above >= line.font_size * 1.25 and gap_below >= line.font_size * 1.1

    if centered and compact_width and (line.bold or larger) and title_ratio >= 0.5:
        return True
    if isolated and compact_width and larger and title_ratio >= 0.75:
        return True
    return False


def page_objects_for_text(lines: list[TextLine], median_font: float, page_width: float) -> list[PageObject]:
    objects: list[PageObject] = []
    for index, line in enumerate(lines):
        previous_line = lines[index - 1] if index > 0 else None
        next_line = lines[index + 1] if index + 1 < len(lines) else None
        kind = "subheading" if is_subheading_line(previous_line, line, next_line, median_font, page_width) else "line"
        objects.append(PageObject(kind=kind, top=line.y0, value=line))
    return objects
...
def process_page(
    ...
    if page_is_toc(lines) or page_has_complex_layout(lines, page_num, page.rect.width):
        return [DocItem(kind="facsimile", page_num=page_num, html_fragment=render_page_facs(page, asset_root, slug))]

    median_font = median(line.font_size for line in lines) if lines else 11.0

    objects = page_objects_for_text(lines, median_font, page.rect.width)
...
def is_subparagraph_transition(previous_item: DocItem | None, item: DocItem) -> bool:
    if item.kind != "ol_item" or item.ordinal is None or previous_item is None:
        return False

    previous_text = (previous_item.text or "").rstrip().lower()
    cue_phrases = (
        "as follows:",
        "held as follows:",
    ...
    if previous_text.endswith(cue_phrases):
        return True

    if previous_item.kind == "ol_item" and previous_item.ordinal is not None:
        return item.ordinal <= previous_item.ordinal
...
def render_body(items: list[DocItem]) -> str:
    ...
    open_list_class: str | None = None
    previous_ordinal: int | None = None
    previous_item: DocItem | None = None

    def close_list() -> None:
        nonlocal open_list_kind, open_list_style, open_list_class, previous_ordinal
        if open_list_kind:
            parts.append(f"</{open_list_kind}>")
        open_list_kind = None
        open_list_style = None
        open_list_class = None
    ...
    for item in items:
        ...
        if item.kind == "ol_item":
            is_subparagraph = is_subparagraph_transition(previous_item, item)
            if (
                not is_subparagraph
                and open_list_kind == "ol"
                and open_list_class == "numbered-list sub-numbered-list"
                and open_list_style == item.list_style
                and previous_ordinal is not None
                and item.ordinal is not None
                and item.ordinal == previous_ordinal + 1
            ):
                is_subparagraph = True

            list_class = "numbered-list sub-numbered-list" if is_subparagraph else "numbered-list"
            li_class = ' class="subparagraph-item"' if is_subparagraph else ""

            needs_new_list = (
                open_list_kind != "ol"
                or open_list_style != item.list_style
                or open_list_class != list_class
            ...
            if needs_new_list:
                ...
                parts.append(f'<ol class="{list_class}"{type_attr}{start_attr}>')
                open_list_kind = "ol"
                open_list_style = item.list_style
                open_list_class = list_class

            item_html = render_inline_html(item)
            if is_subparagraph:
                item_html = f"<em>{item_html}</em>"

            parts.append(f"<li{li_class}>{item_html}</li>")
